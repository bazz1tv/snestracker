...
void Hes_Emu::cpu_write_vdp( int addr, int data )
{
	...
	case 2:
		...
		{
			ddprintf( "VDP not supported: $%02X <- $%02X\n", vdp.latch, data );
		}
		break;
	
	case 3:
		ddprintf( "VDP MSB not supported: $%02X <- $%02X\n", vdp.latch, data );
		break;
	}
}
...
void Hes_Emu::cpu_write_( hes_addr_t addr, int data )
{
	...
	default:
		ddprintf( "unmapped write $%04X <- $%02X\n", addr, data );
		return;
#endif
	}
...
int Hes_Emu::cpu_read_( hes_addr_t addr )
{
	...
	case 0x0002:
	case 0x0003:
		ddprintf( "VDP read not supported: %d\n", addr );
		return 0;
