...
// Add/subtract
	...
	adc_imm: {
		if ( status & st_d )
			ddprintf( "Decimal mode not supported\n" );
		fint16 carry = c >> 8 & 1;
...
// Flags
	...
	delayed_cli:
		ddprintf( "Delayed CLI not supported\n" ); // TODO: implement
		goto loop;
	}
	...
	handle_sei: {
		...
		if ( s_time < 0 )
			goto loop;
		ddprintf( "Delayed SEI not supported\n" ); // TODO: implement
		goto loop;
	}
...
// Illegal
	...
	default:
		...
		ddprintf( "Illegal opcode $%02X at $%04X\n", (int) opcode, (int) pc - 1 );
		illegal_encountered = true;
		goto loop;
	}
	assert( false );
