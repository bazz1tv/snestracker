...
int main( int argc, char** argv )
{
	...
	while ( running )
	{
		...
		/*static unsigned char noisepitch = 0x00, derp = 1;
		static int noisechan=0;
		noisepitch += derp;
		printf ("derp = 0x%02x\n", noisepitch);
		player->spc_emu()->write_flg(noisepitch);
		if (noisepitch >= 0x1f && derp < 0x50)
			derp = 0xff;
		else if (noisepitch == 0x00 || noisepitch > 0x50)
			derp = 1;

		player->spc_emu()->write_noise_on(noisechan);*/
		...
		while ( SDL_PollEvent( &e ) )
		{
			...
			case SDL_KEYDOWN:
				...
				case SDLK_g: 
				{
					...
					//++noisechan;
					//fprintf(stderr, "noisechan = 0x%02x00\n", noisechan);
					break;
				}
				case SDLK_f: 
				{
