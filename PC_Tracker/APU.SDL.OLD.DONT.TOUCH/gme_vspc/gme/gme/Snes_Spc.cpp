...
{
	if ( !enabled )
		ddprintf( "next_tick: %ld, time: %ld", (long) next_tick, (long) time );
...
inline void Snes_Spc::check_for_echo_access( spc_addr_t addr )
{
	if ( !echo_accessed && !(dsp.read( 0x6C ) & 0x20) )
	{
		...
		if ( start <= addr && addr < end ) {
			echo_accessed = true;
			ddprintf( "Read/write at $%04X within echo buffer\n", (unsigned) addr );
		}
	}
}
	...
	else switch ( addr )
	{
		...
		case 0xF3: {
			...
			if ( reg < Spc_Dsp::register_count ) {
				...
				ddprintf( "DSP write to $%02X\n", (int) reg );
			}
			break;
		}
		
		case 0xF0: // Test register
			ddprintf( "Wrote $%02X to $F0\n", (int) data );
			break;
...
blargg_err_t Snes_Spc::play( long count, sample_t* out )
{
	...
	if ( elapsed > 0 )
	{
		ddprintf( "Unhandled instruction $%02X, pc = $%04X\n",
